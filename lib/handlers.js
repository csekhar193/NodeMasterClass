...
handlers.index = function (data, callback) {
	...
	if(data.method = 'get') {
		...
		let templateData = {
			...
			'head.description' : 'We offer free, simple uptime monitoring for HTTP/HTTPS sites of all kinds. When your site goes down, we\'ll send you atext to let you know.',
			'body.class' : 'index'
		}
	...
	if(data.method == 'get') {
		...
		if(trimmedAssetName.length > 0) {
			...
			helpers.getStaticAsset(trimmedAssetName, function(err, data){
				if(!err && data) {
					// Determine the content tpe (default to plain text)
	...
	if( firstname && lastname && phone && password && tosAgreement ) {
		_data.read('users', email, function (err, fileData) {
			if( err ) {
				...
				callback(400, {'Error': 'A User with that email all ready exits.'});
			}
		});
	} else {
